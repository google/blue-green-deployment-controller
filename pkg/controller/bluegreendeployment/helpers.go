...
func newService(b *v1alpha1.BlueGreenDeployment) *corev1.Service {
	return &corev1.Service{
		...
		Spec: corev1.ServiceSpec{
			...
			// Note: Remember to change the ports accordingly.
...
// createReplicaSetsIfNotFound creates blue and green ReplicaSets if they are not found.
func createReplicaSetsIfNotFound(b *v1alpha1.BlueGreenDeployment, c *kubernetes.Clientset) (*appsv1.ReplicaSet, *appsv1.ReplicaSet, error) {
	blueRS, err := createReplicaSetIfNotFound(b, c, BlueRSName, BlueColor)
	if err != nil {
		return nil, nil, fmt.Errorf("when creating blue ReplicaSet, %v", err)
	}

	greenRS, err := createReplicaSetIfNotFound(b, c, GreenRSName, GreenColor)
	if err != nil {
...
// createReplicaSetsIfNotFound creates a ReplicaSet with given color label if it is not found.
func createReplicaSetIfNotFound(b *v1alpha1.BlueGreenDeployment, c *kubernetes.Clientset, rsName, rsColor string) (*appsv1.ReplicaSet, error) {
	ns := b.Namespace
...
// createServiceIfNotFound creates service if it is not found.
func createServiceIfNotFound(b *v1alpha1.BlueGreenDeployment, c *kubernetes.Clientset) (*corev1.Service, error) {
	ns := b.Namespace
...
// replaceInactiveReplicaSet replaces the inactive ReplicaSet by first deleting it (in background) to make room
// for a new ReplicaSet. It then creates the new ReplicaSet based on pod spec of BlueGreenDeployment object.
func replaceInactiveReplicaSet(c *kubernetes.Clientset, b *v1alpha1.BlueGreenDeployment, inactiveRS *appsv1.ReplicaSet) (*appsv1.ReplicaSet, error) {
	...
	err := c.AppsV1().ReplicaSets(ns).Delete(rsName, deleteOptions)
	if err != nil {
		return nil, fmt.Errorf("failed to delete inactive ReplicaSet %q: %v", rsName, err)
	}

	replicaSet, err := createReplicaSetIfNotFound(b, c, rsName, color)
	if err != nil {
...
// defaultFields fills unfilled pod spec fields of BlueGreenDeployment object with default values.
// WARNING: This would not work with multiple API versions. In addition, if user inputs an invalid value for a
// field, there is no validation to catch it. Corresponding ReplicaSet will not be created successfully, thus
// breaking underlying blue-green deployment mechanism.
func defaultFields(c controllerv1alpha1client.ControllerV1alpha1Interface, b *v1alpha1.BlueGreenDeployment) (*v1alpha1.BlueGreenDeployment, error) {
	var err error

	if b, err = updateBlueGreenDeployment(c, b.Name, b.Namespace, func(bgd *v1alpha1.BlueGreenDeployment) {
